...
<%--                <button type="submit"  class="layui-btn"><i class="layui-icon">&#xe615;</i></button>--%>
        ...
        <div style="padding: 0px;">
            ...
            <form id="form-userid" method="post" action="${pageContext.request.contextPath}/user/delSelected" >
            <table class="layui-table"  >
                ...
                <tbody>   <%--   varStatus:遍历的状态信息            --%>
                 <c:forEach varStatus="pid" items="${pageInfos.list}" var="userInf">
                <tr>
                    <c:if test="${pageContext.session.getAttribute('user_identity')==1}">
                    <th><input type="checkbox" name="uid" value="${userInf.id}"> </th>
                    </c:if>
                    <th>${pid.count}</th>
                    <th>${userInf.username}</th>
                    <th>${userInf.login_name}</th>
                    <th>${userInf.password}</th>
                    <c:if test="${pageContext.session.getAttribute('user_identity')==1}">
                    <th><div class="layui-btn-group">
                        <a type="button" class="layui-btn layui-btn-sm" href="${pageContext.request.contextPath}/user/findOne?id=${userInf.id}">编辑</a>
                        <a type="button" class="layui-btn layui-btn-sm" onclick="del(${userInf.id})">删除</a>
                    </div>
                    </th>
                    </c:if>
                </tr>
...
<%--                <div id="paging"> </div>--%>
                ...
                <div>
                    ...
                    <div class="box-tools pull-right">
                        <ul class="pagination">
                            <li ><a href="${pageContext.request.contextPath}/user/findAll?page=1&size=5" aria-label="Previous">首页</a> </li>
                            <li ><a href="${pageContext.request.contextPath}/user/findAll?page=${pageInfos.pageNum-1}&size=5"  >上一页</a> </li>
                       <c:forEach begin="1" end="${pageInfos.pages}"  var="pageNumer">
                           <li><a href="${pageContext.request.contextPath}/user/findAll?page=${pageNumer}&size=5">${pageNumer}</a></li>
                       </c:forEach>
                            <li ><a href="${pageContext.request.contextPath}/user/findAll?page=${pageInfos.pageNum+1}&size=5"  >下一页</a> </li>
                            <li ><a href="${pageContext.request.contextPath}/user/findAll?page=${pageInfos.pages}&size=5"  >尾页</a> </li>
                        </ul>
                    </div>
                </div>
            </form>
...
<%--    删除提示--%>
    function del(aid) {
        ...
        if (flag){
            location.href="${pageContext.request.contextPath}/user/deleteById?id="+aid;
